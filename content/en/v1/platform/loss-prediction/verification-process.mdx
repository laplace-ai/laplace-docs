...
If the operator does not act on a Stage 1 task within **4 hours**, the system automatically advances the CTRC to Stage 2. This ensures that cargo is never stuck in a pending state indefinitely — the verification process continues even if the operator is unavailable or misses the task.

<Callout type="warning">
An auto-advanced CTRC is treated as **unverified** — it did not receive explicit confirmation or denial. This is logged for analysis and may indicate operational gaps at the unit.
</Callout>
...
flowchart TD
    ...
    G["⏱️ Stage 1 no action\nsince 10:00"] --> H{"14:00 — 4 hours\nelapsed"}
    H --> I["📋 Stage 2 created\nAuto-advanced"]
```

<Callout type="warning">
If cargo is marked as **not found**, the cooldown is bypassed entirely. Stage 2 is created immediately because the situation requires urgent follow-up. If the operator takes **no action at all**, the system auto-advances after 4 hours.
</Callout>
...
### If the operator takes no action at Stage 2

If the operator does not act on a Stage 2 task within **5 hours**, the system automatically flags the CTRC as a **potential loss**. At this point, the cargo has gone through the entire verification window without any confirmation — an investigation should be initiated.

<Callout type="danger">
A Stage 2 timeout is a strong signal of a potential loss. The cargo passed through two verification windows (Stage 1 + Stage 2) without being located by any operator.
</Callout>
...
- **No action (timeout)** → System flags as potential loss after 5 hours. Investigation required.
...
| **Inaction timeout** | Stage 1: 4 hours → auto-advance to Stage 2. Stage 2: 5 hours → flagged as potential loss |
