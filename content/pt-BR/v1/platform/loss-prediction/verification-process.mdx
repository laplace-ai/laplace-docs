...
### Se o operador não fizer nada

Se o operador não agir sobre uma tarefa da Etapa 1 dentro de **4 horas**, o sistema avança automaticamente o CTRC para a Etapa 2. Isso garante que nenhuma carga fique presa em estado pendente indefinidamente — o processo de verificação continua mesmo que o operador esteja indisponível ou não veja a tarefa.

<Callout type="warning">
Um CTRC avançado automaticamente é tratado como **não verificado** — não recebeu confirmação nem negação explícita. Isso é registrado para análise e pode indicar gaps operacionais na unidade.
</Callout>
...
flowchart TD
    ...
    G["⏱️ Etapa 1 sem ação\ndesde 10:00"] --> H{"14:00 — 4 horas\ndecorridas"}
    H --> I["📋 Etapa 2 criada\nAvanço automático"]
```

<Callout type="warning">
Se a carga for marcada como **não encontrada**, o cooldown é ignorado completamente. A Etapa 2 é criada imediatamente. Se o operador **não fizer nada**, o sistema avança automaticamente após 4 horas.
</Callout>
...
### Se o operador não fizer nada na Etapa 2

Se o operador não agir sobre uma tarefa da Etapa 2 dentro de **5 horas**, o sistema automaticamente sinaliza o CTRC como **potencial extravio**. Nesse ponto, a carga passou por toda a janela de verificação sem nenhuma confirmação — uma investigação deve ser iniciada.

<Callout type="danger">
Um timeout na Etapa 2 é um sinal forte de potencial extravio. A carga passou por duas janelas de verificação (Etapa 1 + Etapa 2) sem ser localizada por nenhum operador.
</Callout>
...
- **Sem ação (timeout)** → Sistema sinaliza como potencial extravio após 5 horas. Investigação necessária.
...
flowchart TD
    ...
    subgraph s1["📋 Etapa 1 · Recebimento"]
        ...
        I -->|"Sem ação\n⏱️ 4 horas"| K2["⏱️ Avanço automático"]
    end
...
| **Timeout por inação** | Etapa 1: 4 horas → avanço automático para Etapa 2. Etapa 2: 5 horas → sinalizado como potencial extravio |
